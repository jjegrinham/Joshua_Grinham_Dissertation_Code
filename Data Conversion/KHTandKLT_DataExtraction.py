...
class ProgramVariables:
    ...
    hightempdata = directory+'/'+subdirectory_1+'/'+'hightempdata.txt'
    lowtempdata = directory+'/'+subdirectory_1+'/'+'lowtempdata.txt'

    startpos = {hightempdata: -1, lowtempdata: -1}
    datatype = {hightempdata: 'hightemp', lowtempdata: 'lowtemp'}
    headings = ['Sample ID', 'Measuring Mode', 'Measurement Order Index', 'Field Strength (A/m)',
                'Field Frequency (Hz)', 'Temperature (ºC)', 'Raw Sample Susceptibility (In-Phase)',
    ...
    welcomemessage = '''This program takes .KHT and .KLT thermosusceptibility files
    ...
    per data type. Additionally a EXCEL (.xlsx) spreadsheet can be generated to view the data processed.'''
...
def main():
    ...
    while fileselect == '':
        try:
            filepath = easygui.fileopenbox(title='Select a file', filetypes=['*.KHT', '*.KLT'], multiple=True)
        except Exception as e:
            print(e)
        for f in filepath:
            print(f)
...
def fileread(f):
    with open(f, 'r') as datafile:
        ...
        if results[n] == 'k(HT)':
            databasefile = pro.hightempdata
        elif results[n] == 'k(LT)':
            databasefile = pro.lowtempdata
        else:
            valid = False
        if valid:
